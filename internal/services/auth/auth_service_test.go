...
func (r *fakeOTPRepo) Create(_ context.Context, o *models.OTPToken) error {
	...
	for _, t := range r.tokens {
		if t.Phone == o.Phone && t.Purpose == o.Purpose { t.Used = true }
	}
...
func TestRegister_Success(t *testing.T) {
	...
	if msg == "" { t.Fatal("expected success message") }

	u, err := users.FindByPhone(ctx, "+2348012345678")
	if err != nil { t.Fatalf("user not persisted: %v", err) }
	if u.FirstName != "Ada" { t.Errorf("FirstName = %q, want Ada", u.FirstName) }
	if u.PasswordHash == "Password123!" { t.Fatal("password stored as plain text") }
	if u.KYCTier != models.KYCTierUnverified { t.Errorf("KYCTier = %d, want 0", u.KYCTier) }

	sms.mu.Lock()
	sent := len(sms.sent)
	sms.mu.Unlock()
	if sent == 0 { t.Fatal("OTP SMS not sent after registration") }
}
...
func TestRegister_DuplicatePhone(t *testing.T) {
	...
	if _, err := svc.Register(ctx, req); err != nil { t.Fatalf("first register: %v", err) }
	if _, err := svc.Register(ctx, req); err == nil { t.Fatal("duplicate phone should fail") }
}
...
func TestLogin_Success(t *testing.T) {
	...
	if tokens.AccessToken == ""  { t.Fatal("access token empty") }
	if tokens.RefreshToken == "" { t.Fatal("refresh token empty") }
	if tokens.AccessToken == tokens.RefreshToken { t.Fatal("access and refresh tokens must differ") }
}
...
func TestSetPIN_Success(t *testing.T) {
	...
	if updated.PinHash == "" { t.Fatal("PinHash not stored") }
...
func TestRefreshToken_Success(t *testing.T) {
	...
	if err != nil { t.Fatalf("RefreshToken error: %v", err) }
	if newTokens.AccessToken == "" { t.Fatal("new access token empty") }
}
...
func TestUpgradeKYC_BVN(t *testing.T) {
	...
	if err != nil { t.Fatalf("UpgradeKYC BVN error: %v", err) }
	if tier != models.KYCTierOne { t.Errorf("tier = %d, want KYCTierOne", tier) }

	updated, _ := users.FindByID(ctx, u.ID.String())
	if updated.KYCTier != models.KYCTierOne { t.Errorf("stored tier = %d, want KYCTierOne", updated.KYCTier) }
	if updated.BVN != "12345678901" { t.Errorf("BVN = %q, want 12345678901", updated.BVN) }
}
...
func TestUpgradeKYC_NIN_GoesToTierTwo(t *testing.T) {
	...
	if err != nil { t.Fatalf("UpgradeKYC NIN error: %v", err) }
	if tier != models.KYCTierTwo { t.Errorf("tier = %d, want KYCTierTwo", tier) }
}
